...
function calcAdvanced(slug:string,v:Record<string,string>,currency:string):Result[]{const n=(k:string)=>num(v[k]);const fmt=(x:number)=>money(x,currency);const r=n('rate')/100;const y=n('years');
  ...
  if(slug==='ppf-calculator'){const P=n('contribution'),rate=r,months=Math.round(y*12),annualRate=rate;let bal=0;for(let i=0;i<Math.max(1,y);i++)bal=(bal+P)*(1+annualRate);return [{label:'Estimated Maturity Value',value:fmt(bal)},{label:'Total Contributions',value:fmt(P*Math.max(1,y))},{label:'Estimated Interest',value:fmt(bal-P*Math.max(1,y))}]}
